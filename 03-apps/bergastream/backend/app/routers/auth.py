...
async def register(body: RegisterRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    token, expires_at = await auth_service.create_session(db, user.id, request.headers.get("user-agent"))

    from datetime import timezone
    expires_in = int((expires_at - expires_at.replace(tzinfo=timezone.utc)).total_seconds()) if expires_at.tzinfo else settings.jwt_expire_minutes * 60
